...
def birthDay_calc(years,months,days,year,month,date):
    import datetime
    today = datetime.date(years,months,days)
    day_O_birth = datetime.date(year,month,date)
    years = ((today-day_O_birth).total_seconds()/ (365.242*24*3600))
    yearsInt = int(years)
...
birthDay_calc(c_year,c_month,c_date, b_year, b_month,b_date)
